...
Integrates LLM, Emotional Intelligence and Reinforcement Learning
"""
...
Response:"""
    ...
    def _update_user_profile(self, user_input: str, emotional_context: Dict[str, Any]):
        ...
        if not self.current_user_profile.get('user_name'):
            import re
            # Look for introduction patterns
            intro_patterns = [
                r"(?:hi|hello|hey)\s+(?:i'm|i am|my name is|call me)\s+([A-Za-z]+)",
                r"(?:i'm|i am|my name is|call me)\s+([A-Za-z]+)",
                r"([A-Za-z]+)\s+here",
                r"this is ([A-Za-z]+)"
            ]
            ...
            for pattern in intro_patterns:
                ...
                if match:
                    ...
                    # Basic validation - name should be 2-20 chars, not common words
                    if 2 <= len(potential_name) <= 20 and potential_name.lower() not in ['the', 'and', 'but', 'for', 'are', 'you', 'how', 'what', 'when', 'where', 'why']:
                        self.current_user_profile['user_name'] = potential_name
                        print(f"✅ Learned user name: {potential_name}")
                        break
